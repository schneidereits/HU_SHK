...
_(ie because NA filter removed some layers and dates dont correspond when using the previous index)_
```{r, results='hide'}
tsi_stack <- c(tsi_list[[1]][[c(550)]],
               tsi_list[[2]][[c(718)]],
               tsi_list[[3]][[c(720)]],
               tsi_list[[4]][[c(720)]])
...
### Naming ROIs
```{r generation of sample points, results='hide'}
...
```{r generation of sample points, results='hide', eval=FALSE}
# select the ROI for each location type
roi_list <- list()
...
```{r generation of sample points, results='hide', eval=TRUE}
#load sampled points
roi_list <- list()
